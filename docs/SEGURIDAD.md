...
Se retiró `.env` del seguimiento y se sustituyó `config/config.json` por `config/config.js` basado en variables de entorno. Se reemplazó el historial principal por un commit limpio. Las credenciales expuestas deben considerarse comprometidas aunque el servicio original ya no esté activo. No se reproducen en esta documentación. Las copias externas y las referencias o cachés retenidas por GitHub no se eliminan mediante una reescritura y pueden requerir intervención de GitHub Support.
...
4. El historial principal se sustituyó con autorización del propietario. Cualquier copia local anterior debe clonarse de nuevo y no debe volver a publicarse. Las referencias de PR y vistas de commits retenidas por GitHub deben solicitarse a GitHub Support si continúan accesibles.

La API y PostgreSQL permanecen detenidos según su propietario; no se modificaron servicios de Render ni se rotaron sus claves. Antes de reactivar o desplegar, renueva las credenciales y configura DB_NAME, DB_USER, DB_PASSWORD, DB_HOST, DB_PORT y un JWT_SECRET aleatorio de al menos 32 caracteres. El arranque rechaza una configuración incompleta o la clave de ejemplo. Prepara primero las variables privadas y Node.js 22/24. No reutilices credenciales históricas ni restaures un `.env` antiguo.
